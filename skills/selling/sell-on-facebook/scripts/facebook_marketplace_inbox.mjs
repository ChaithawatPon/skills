...
function draftAutoReply(thread, facts = []) {
  ...
  if (!buyerText) return ''
  const fact = matchListingFact(thread.listingTitle, facts)
  const factReply = answerFromListingFact(buyerText, fact, particle)
...
function isAutoReplyEligible(thread, state = {}, facts = []) {
  if (!thread.isSellerListing) return { eligible: false, reason: 'not a verified seller listing' }
  if (!thread.listingActive) return { eligible: false, reason: 'listing is not active' }
  const messages = thread.messages.filter(substantive)
  const latest = messages.at(-1)
  ...
  return {
    ...
    reply: draftAutoReply(thread, facts),
...
async function sendAutoReply({ page, thread, state, now = Date.now(), facts = loadListingFacts() }) {
  const decision = isAutoReplyEligible(thread, state, facts)
...
async function scanInbox({ limit = 30 } = {}) {
  ...
  try {
    ...
    for (const row of rows.slice(0, limit)) {
      ...
      const replyDecision = isAutoReplyEligible(thread, state, facts)
  ...
  try {
    ...
    for (const row of rows.slice(0, limit)) {
      ...
      if (replyDecision.eligible) {
        const result = await sendAutoReply({ page, thread, state, now: Date.now(), facts })
...
async function runSelfTest() {
  ...
  const cutterFacts = [{
    ...
    meetup: 'นัดรับกรุงเทพ รายละเอียดนัดตอนทัก',
    shipping: 'ไม่รวมส่ง เว้นแต่ตกลงกัน',
    size: 'A4',
  }]
  const priceThread = {
    ...replyThread,
    listingTitle: 'แท่นตัดกระดาษ A4',
    messages: [{ author: 'buyer', text: 'ราคาเท่าไหร่', timestamp: now, timestampText: 'Saturday 10:00am' }],
  }
  assert(draftAutoReply(priceThread, cutterFacts) === '220 บาทครับ', 'listing-facts price reply failed')
